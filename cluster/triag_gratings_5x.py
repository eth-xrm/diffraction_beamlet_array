        ...
        "elements": [
            {
                ...
                "dc": [0.0, 1],
                "z_start": float(d1),
                "thickness": float(thick*5),
                "nr_steps": 50,
                "x_positions": [0.0],
                "substrate_thickness": (500) * 1e-6 - float(thick*5),
                "mat_a": ["Si", 2.34],
                "mat_b": ["Au", 19.32],
...
sim_path_tri = multisim.setup_simulation(config_dict, Path("."), simulations_dir)
for i in range(config_dict["multisource"]["nr_source_points"]):
    os.system(f"CUDA_VISIBLE_DEVICES=0 path/to/python/python_packages/rave-sim/fast-wave/build-Release/fastwave -s {i} {sim_path_tri}")
